...
- F09.4: el anillo/contorno de cooldown alrededor del jugador (`NV.drawSpecialCooldown`) fue eliminado globalmente (gameplay + preview). El estado de la habilidad vive en el botón ESPECIAL móvil (anillo de progreso, segundos restantes y LISTO) y en el slot canvas del panel (`drawWeaponHUD`).
