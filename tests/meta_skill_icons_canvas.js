// Tests Bloque 1: helper canvas de iconos SVG-approved de mejoras/habilidades.
...
vm.runInNewContext(fs.readFileSync('js/render/metaSkillIcons.js', 'utf8'), sbx, { filename: 'js/render/metaSkillIcons.js' });
const NV = sbx.window.NV;

t('expone drawMetaSkillIcon + 13 ids aprobados', () => {
  const ids = ['damage','speed','hp','armor','luck','crit','dodge','regen','greed','meteor','phase','bulwark','hivemind'];
  if (typeof NV.drawMetaSkillIcon !== 'function') throw new Error('falta drawMetaSkillIcon');
  if (NV.META_SKILL_ICON_IDS.length !== 13) throw new Error('cantidad ' + NV.META_SKILL_ICON_IDS.length);
  for (const id of ids) if (!NV.META_SKILL_ICON_IDS.includes(id)) throw new Error('falta ' + id);
});

t('cada icono renderiza strokes/fills en canvas sin Path2D', () => {
  for (const id of NV.META_SKILL_ICON_IDS) {
    const ctx = mkCtx();
    NV.drawMetaSkillIcon(ctx, id, 16, 16, 24, { glow: 2 });
...
t('metaSkillIconToDataURL usa canvas y devuelve data url', () => {
  const url = NV.metaSkillIconToDataURL('meteor', 48);
  if (!url.startsWith('data:image/png;base64,')) throw new Error(url);
});

console.log('RESULT meta_skill_icons_canvas: pass=' + pass + ' fail=' + fail);
