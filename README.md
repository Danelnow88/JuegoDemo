...
- **Stage 3 Dynamic World View default**: en móvil landscape, la vista/arena dinámica se activa por defecto y llena el ancho físico sin gutters legacy; desktop conserva `900×520`. `?dynamicView=1` queda como alias compatible y `?dynamicView=0` fuerza contain legacy para debug.
...
### v61 — Más presencia rítmica: shake global reforzado
- **Screen-shake más perceptible**: `NV.rhythmShakeBoost` sube el impulso máximo de `0.16` a `0.26` y baja levemente el umbral de golpe fuerte, manteniendo cooldown y requisito de onset reciente.
